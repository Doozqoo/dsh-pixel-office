...
| 设置 | 保留原生设置面板并重绘为像素风；插件不再向设置面板添加独立分区 |
| 主题配置 | 像素皮肤始终启用，动效固定为 `OVERDRIVE`，空间网格固定开启 |
| 持久化 | 工位布局、便利贴摆放与自定义文字存进 `localStorage`，刷新后保持原样 |
...
| 动效档位 | 固定使用 `OVERDRIVE`；仍尊重系统 `prefers-reduced-motion` |

## 固定主题配置

插件不再注册「赛博工位」设置分区。像素皮肤、`OVERDRIVE` 动效和空间网格均固定开启，旧版写入 `localStorage` 的主题配置会被忽略；工位布局、便利贴摆放和自定义文字仍会保留。

> 如果想恢复原版界面，请运行 `npx @deepseek-ai/dsh plugin --profile web remove dsh-client-pixel-office` 并重启 `dsh web`。
